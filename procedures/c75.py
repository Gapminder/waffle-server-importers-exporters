...
def get_pop_for_region(min_year, max_year, regions_list, region_id, pop_version):
    region_pop = []
    print "Calculating pop values for Europe"
    for year in range(min_year, max_year):
        region_pop_per_year = [val for val in regions_list if int(val['Year']) == year]
        region_per_year_sum = sum(val['Population with interpolations'] for val in region_pop_per_year)

    reg_pop = dict()
    reg_pop['version'] = pop_version
    reg_pop['Area'] = region_id
    reg_pop['Year'] = year
    reg_pop['Population with interpolations'] = region_per_year_sum
    reg_pop['Source'] = 'No Data'
...
def cal_pop_for_regions(pop, entities_geo, pop_version):
    pop_for_region = dict()
    ...
    pop_for_region['eur'] = get_pop_for_region(min_year_eur, max_year_eur, entity_regions_pop['eur'], 'eur', pop_version)

    min_year_ame = get_min_year_by_region(entity_regions_pop['ame'])
    max_year_ame = get_max_year_by_region(entity_regions_pop['ame'])
    pop_for_region['ame'] = get_pop_for_region(min_year_ame, max_year_ame, entity_regions_pop['ame'], 'ame', pop_version)

    min_year_afr = get_min_year_by_region(entity_regions_pop['afr'])
    max_year_afr = get_max_year_by_region(entity_regions_pop['afr'])
    pop_for_region['afr'] = get_pop_for_region(min_year_afr, max_year_afr, entity_regions_pop['afr'], 'afr', pop_version)
    ...
    pop_for_region['asi'] = get_pop_for_region(min_year_asi, max_year_asi, entity_regions_pop['asi'], 'asi', pop_version)
